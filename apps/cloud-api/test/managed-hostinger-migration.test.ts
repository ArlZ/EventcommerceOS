...
describe('managed Hostinger migration preflight', () => {
  it('only activates for the managed Cloud API target', () => {
    ...
    expect(
      isManagedHostingerCloudApi({ HOSTINGER_APP_TARGET: 'control-web' }),
    ).toBe(false);
    expect(
      isManagedHostingerCloudApi({ HOSTINGER_APP_TARGET: ' cloud-api ' }),
    ).toBe(true);
  });

  it(
    'runs the repository migration script before managed Cloud startup',
    () => {
  ...
  it(
    'orders migration before Nest bootstrap on the configured managed entry path',
    async () => {
      const order: string[] = [];
      const appRoot = resolve('/workspace', 'apps', 'cloud-api');
