...
export class PosMenuService {
  ...
  async install(snapshot: PosMenuSnapshot): Promise<PosMenuSnapshot> {
    return this.database.transaction((client) => this.installOne(client, snapshot));
  }

  async installBatch(snapshots: PosMenuSnapshot[]): Promise<PosMenuSnapshot[]> {
    if (snapshots.length === 0) return [];
    const keys = new Set<string>();
    for (const snapshot of snapshots) {
      const key = `${snapshot.eventId}:${snapshot.salesLocationId}`;
      if (keys.has(key)) {
        throw new ConflictException('menu snapshot batch contains duplicate sales location scope');
      }
      keys.add(key);
    }
    ...
    return this.database.transaction(async (client) => {
      const ordered = [...snapshots].sort((left, right) => {
        const leftKey = `${left.eventId}:${left.salesLocationId}`;
        const rightKey = `${right.eventId}:${right.salesLocationId}`;
        return leftKey.localeCompare(rightKey);
      });
      const installed: PosMenuSnapshot[] = [];
      for (const snapshot of ordered) {
        installed.push(await this.installOne(client, snapshot));
      }
      return installed;
    });
  }
