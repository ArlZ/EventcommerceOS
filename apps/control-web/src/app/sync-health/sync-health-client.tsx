...
function deviceStatus(device: DeviceCloudStatus): {
  ...
  if (status === 'STALE') {
    return {
      label: device.lastSeenAt === null ? 'Not reported yet' : 'Not reporting',
      tone: 'danger',
      detail:
        device.lastSeenAt === null
          ? 'This provisioned register has not reported a heartbeat yet. Local selling status cannot be confirmed from Cloud telemetry.'
          : `Last heartbeat ${ageLabel(device.lastSeenAt)}. Local selling status cannot be confirmed from Cloud telemetry.`,
    };
  }
  ...
  return (
    ...
    >
      ...
      <section className="ec-kpi-grid" aria-label="Device sync summary">
        <SyncMetric label="Registers known" value={devices.length.toString()} />
            ...
            <article className="ec-panel" key={device.deviceId}>
              <div className="ec-panel-heading">
                <div className="ec-entity-heading">
                  ...
                  <p>
                    {device.lastSeenAt === null
                      ? 'No register heartbeat observed yet'
