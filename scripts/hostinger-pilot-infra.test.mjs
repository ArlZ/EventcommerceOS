...
const compose = readFileSync(
  resolve(root, 'infra/hostinger/pilot/docker-compose.yml'),
  'utf8',
);
...
const restore = readFileSync(
  resolve(root, 'infra/hostinger/pilot/restore-check.sh'),
  'utf8',
);
const envExample = readFileSync(
  resolve(root, 'infra/hostinger/pilot/.env.example'),
  'utf8',
);
...
test(
  'Hostinger pilot exposes applications only through external Traefik HTTPS routing',
  () => {
...
test(
  'Hostinger deployment binds build, runtime, migration and smoke to exact release identity',
  () => {
...
test(
  'Hostinger backup tooling hashes dumps, retains locally, and supports isolated restore checking',
  () => {
...
test(
  'Hostinger example environment contains placeholders rather than committed credentials',
  () => {
    assert.match(
      envExample,
      /POSTGRES_PASSWORD=replace-with-64-lowercase-hex-characters/,
    );
