...
test('Hostinger deployment workflow is manual, immutable and bound to exact main SHA', () => {
  ...
  assert.match(workflow, /uses: actions\/checkout@d23441a48e516b6c34aea4fa41551a30e30af803/);
  assert.match(workflow, /persist-credentials: false/);
  assert.match(workflow, /ref: \$\{\{ github\.sha \}\}/);
  assert.match(workflow, /test "\$\(git rev-parse HEAD\)" = "\$RELEASE_COMMIT"/);
  assert.match(workflow, /uses: hostinger\/deploy-on-vps@b0868cec74bfa63af6bfa7bdb0dfee2f9bef13ea/);
