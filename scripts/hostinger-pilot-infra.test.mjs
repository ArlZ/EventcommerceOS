...
const workflow = readFileSync(
  '.github/workflows/hostinger-pilot-deploy.yml',
  'utf8',
);
...
test(
  'Hostinger pilot runs exact-release migrations before Cloud traffic',
  () => {
...
test(
  'Hostinger pilot preserves bounded abuse semantics and sandbox M-PESA',
  () => {
...
test(
  'Hostinger deployment workflow is manual, immutable and bound to exact main SHA',
  () => {
    ...
    assert.match(workflow, /test "\$RELEASE_COMMIT" = "\$WORKFLOW_SHA"/);
    assert.match(
      workflow,
    ...
    assert.match(
      workflow,
      /docker-compose-path: infra\/hostinger\/pilot\/docker-compose\.yml/,
    );
  },
);
