...
Prevent previously applied SQL migrations from silently changing after deployment, and ensure production migration/administrative tooling never guesses which database to modify.
...
4. Require explicit `DATABASE_URL` for Cloud production migration/admin tooling.
...
- Production Cloud migration, operator-auth and Edge-credential tools require explicit `DATABASE_URL`.
