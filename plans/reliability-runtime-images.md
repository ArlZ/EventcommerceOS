...
5. Add a CI `runtime-images` job that:
   - builds Cloud API and Event Edge images from the exact PR candidate;
   - runs their database migrations from the built images;
   - boots both images against CI Postgres;
   - polls `/health` and requires the exact candidate SHA from each service;
   - builds Control Web with an explicit Cloud API public URL.
...
- Do not claim a registry publication or deployment that has not actually occurred.
