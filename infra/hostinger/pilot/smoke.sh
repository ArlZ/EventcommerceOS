...
if (
  ...
  throw new Error(
    `${label} must be a canonical HTTPS origin without credentials, path, query or fragment`,
  );
}
NODE
}
...
if (body.service !== expectedService) {
  throw new Error(`expected ${expectedService}, got ${body.service}`);
}
if (body.releaseCommit !== expectedRelease) {
  throw new Error(
    `${expectedService} release mismatch: expected ${expectedRelease}, got ${body.releaseCommit}`,
  );
}
NODE
